 ...
 *
 */
public class ChatView extends AbstractTabbedView<Chat, ConversationView> implements ChatManagerListener, ChatStateListener {
	...
	public ChatView(final ChatContextManager contextManager, VCardCache cardCache) {
		...
		JXButton sendButton = new JXButton("Send");
		
		JXPanel buttonPane = new JXPanel(new FlowLayout(FlowLayout.TRAILING));
		buttonPane.add(closeButton);
		buttonPane.add(sendButton);
		add(buttonPane, BorderLayout.SOUTH);
		*/
        ActionContainerFactory acf = new ActionContainerFactory();
        IconSet icons = AbstractIconSetFactory.getInstance(LiquidicityIconSetFactory.class.getName()).getDefaultIconSet();

//        CloseChatView closeChatView = new CloseChatView(((OpenChatView) ActionManager.getInstance().getAction("closeTab")).getChatFrame());
...
//        ShowStatusBarAction viewStatusBar = new ShowStatusBarAction();
        ShowStatusBarAction viewStatusBar = (ShowStatusBarAction) ActionManager.getInstance().getAction("chat.viewStatusBar");
        viewStatusBar.addStatusBar(statusBar);
//        ActionManager.getInstance().addAction(getId() + ".viewStatusBar", viewStatusBar);
        
        CloseTab<ChatView, Chat> closeTab = (CloseTab<ChatView, Chat>) ActionManager.getInstance().getAction("chat.closeTab");
...
//        ActionManager.getInstance().addAction(getId() + ".closeTab", closeTab);
        ...
        CloseOtherTabs<ChatView, Chat> closeOtherTabs = (CloseOtherTabs<ChatView, Chat>) ActionManager.getInstance().getAction("chat.closeOtherTabs");
...
//        ActionManager.getInstance().addAction(getId() + ".closeOtherTabs", closeOtherTabs);
        ...
        CloseAllTabs<ChatView, Chat> closeAllTabs = (CloseAllTabs<ChatView, Chat>) ActionManager.getInstance().getAction("chat.closeAllTabs");
...
//        if (!createdLocally) {
//            XmppContact contact = (XmppContact) contactStore.getContact(StringUtils.parseBareAddress(chat.getParticipant()));
//            chats.put(contact, chat);
            ...
            ViewFrame<ChatView> chatFrame = ((OpenChatView) ActionManager.getInstance().getAction("chat")).getChatFrame();
